...
import { RegularSeasonGamesEffects } from "./regular-season-games.effect"
...
const RegularSeasonGamesServiceMock = {
    findAll(){
        return of(list)
    }
}

describe('RegularSeasonGamesEffects',()=>{
    ...
    beforeEach(async () => {
        await TestBed.configureTestingModule({
          ...
          imports:[],
          providers: [
            { provide: ReagularSeasonService, useValue: RegularSeasonGamesServiceMock },
          ],
        })
        .compileComponents();
    });

    it('should correctly call RegularSeasonGamesEffects', () => {
        ...
        const { cold, hot, expectObservable, expectSubscriptions, flush, time, animate } = helpers;
            actions$ = of({ type: RegularSeasonGamesActionTypes.GET_REGULAR_SEASON_GAMES });
            // mock the service to prevent an HTTP request
            const regularseasongamesServiceSpy = TestBed.inject(ReagularSeasonService);
            regularseasongamesServiceSpy.findAll();
            const effects = new RegularSeasonGamesEffects(actions$, regularseasongamesServiceSpy);            

            // subscribe to the Effect stream and verify it dispatches a SUCCESS action
            effects.selectRegularSeasonGames$.subscribe(action => {
            expect(action).toEqual({
                type: RegularSeasonGamesActionTypes.GET_REGULAR_SEASON_GAMES_SUCCESS,
                regularseasongames: list,
            });
        });
    })
  })
