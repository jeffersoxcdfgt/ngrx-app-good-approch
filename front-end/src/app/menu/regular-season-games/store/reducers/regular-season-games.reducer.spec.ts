import * as fromReducer from './regular-season-games.reducer';
import { regularseasongamesGetAll, regularseasongamesGetAllSuccess } from '../actions/regular-season-games.action';
...
describe('RegularSeasonGamesReducer', () => {
  ...
  describe('regularseasongamesGetAll action', () => {
    it('should retrieve all regular seasons and update the state in an immutable way', () => {
      const newState: Array<ReagularSeasonGame>= [];
      const action = regularseasongamesGetAll();
      const state = fromReducer.reducer(initialState, action);

      expect(state).toEqual({regularseasongames:newState,loading:true});
      expect(state).not.toBe(initialState);
    });
  });

  describe('regularseasongamesGetAllSuccess action', () => {
    it('should retrieve all regular season and update the state in an immutable way', () => {
      const newState: Array<ReagularSeasonGame>= [
        {
            id:1,
      ...
      const action = regularseasongamesGetAllSuccess({regularseasongames:newState});
      const state = fromReducer.reducer(initialState, action);
      expect(state).toEqual({regularseasongames:newState,loading:false});
      expect(state).not.toBe(initialState);
    });
  });
